...
tab_names = ["Today", "Trade Board", "Explore", "Movers", "Watchlist", "Charts", "Learn", "Disclaimer", "Public View"] if view_mode == "Execution" else ["Today", "Trade Board", "Explore", "Movers", "Watchlist", "Charts", "Market", "Structure Changes", "Learn", "Disclaimer", "Public View"]
...
with tabs[tab_offset + 2]:
    st.markdown("### 📱 Public View Preview")
    st.caption("Mobile-first public-facing preview using your existing dashboard data. Buy/Sell/Short language is hidden here.")

    left_ctrl, right_ctrl = st.columns([1, 1])
    with left_ctrl:
        public_count = st.slider("Number of cards", min_value=5, max_value=30, value=12, step=1, key="public_view_count")
    with right_ctrl:
        public_stage_filter = st.multiselect(
            "Stages to show",
            options=["Stage 1", "Stage 2", "Stage 3", "Stage 4"],
            default=["Stage 2", "Stage 3"],
            key="public_stage_filter",
        )

    public_df = combined.copy()
    public_df = ensure_label(public_df)
    public_df = ensure_rank_column(public_df)
    if public_stage_filter and "stage" in public_df.columns:
        public_df = public_df[public_df["stage"].isin(public_stage_filter)].copy()
    public_df = safe_sort_values(public_df, ["current_rank", "Company Name"], [True, True]).head(public_count)

    stage_summary = stage_count_summary(combined)
    leader_sector = "-"
    try:
        _stage2_sector = (
            combined[combined["stage"].eq("Stage 2")]
            .groupby("Industry")
            .size()
            .reset_index(name="count")
            .sort_values("count", ascending=False)
        )
        if not _stage2_sector.empty:
            leader_sector = str(_stage2_sector.iloc[0]["Industry"])
    except Exception:
        leader_sector = "-"

    st.markdown("#### Today in 60 Seconds")
    m1, m2, m3, m4 = st.columns(4)
    with m1:
        st.metric("Stage 2", int(stage_summary.get("Stage 2", 0)))
    with m2:
        st.metric("Stage 3/4", int(stage_summary.get("Stage 3", 0) + stage_summary.get("Stage 4", 0)))
    with m3:
        st.metric("Preview Cards", len(public_df))
    with m4:
        st.metric("Leader Sector", leader_sector)

    st.markdown("---")
    st.markdown("#### Swipe-style structure cards")
    ...
    if public_df.empty:
        ...
        for _, row in public_df.iterrows():
            with st.container(border=True):
                ...
                with top_cols[0]:
                    st.markdown(f"#### {stock_display_label(row)}")
                    st.caption(f"{row.get('Industry', '-')} • {row.get('stage', '-')} • {stage_primary_label(str(row.get('stage', '')))}")
                with top_cols[1]:
                    rank_val = pd.to_numeric(row.get("current_rank"), errors="coerce")
                    st.metric("Rank", f"#{int(rank_val)}" if pd.notna(rank_val) else "-")

                chip_cols = st.columns(3)
                with chip_cols[0]:
                    st.info(structure_category(row))
                with chip_cols[1]:
                    rs3 = pd.to_numeric(row.get("rs_3m_pct"), errors="coerce")
                    st.metric("RS 3M", f"{rs3:+.1f}%" if pd.notna(rs3) else "-")
                with chip_cols[2]:
                    rs6 = pd.to_numeric(row.get("rs_6m_pct"), errors="coerce")
                    st.metric("RS 6M", f"{rs6:+.1f}%" if pd.notna(rs6) else "-")

                st.write(signal_summary(row))

                ticker = str(row.get("ticker", ""))
                daily_chart = resolve_chart_path(daily_dir, ticker, "_daily.png")
                weekly_chart = resolve_chart_path(weekly_dir, ticker, "_weekly.png")
                chart_cols = st.columns(2)
                with chart_cols[0]:
                    if daily_chart:
                        st.image(str(daily_chart), caption="Daily chart", use_container_width=True)
                    else:
                        st.caption("Daily chart not found")
                with chart_cols[1]:
                    if weekly_chart:
                        st.image(str(weekly_chart), caption="Weekly chart", use_container_width=True)
                    else:
                        st.caption("Weekly chart not found")

    st.markdown("---")
    st.caption("This public preview is for market-structure analytics only. It does not provide investment advice or buy/sell/hold recommendations.")
