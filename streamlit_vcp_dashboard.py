...
def get_last_updated(file_path: str) -> str:
    p = Path(file_path)
    if not p.exists(): return "Not available"
...
def add_safe_labels(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "daily_setup_bucket" in out.columns:
        out["daily_setup_label"] = out["daily_setup_bucket"].map({"breakout_today":"Breakout setup detected","near_pivot":"Near pivot zone","forming_vcp":"Setup forming","watchlist":"Watchlist"}).fillna(out["daily_setup_bucket"])
    if "weekly_setup_bucket" in out.columns:
        out["weekly_setup_label"] = out["weekly_setup_bucket"].map({"weekly_breakout":"Weekly breakout structure","weekly_near_pivot":"Weekly near pivot zone","weekly_forming":"Weekly structure forming","weekly_watchlist":"Weekly watchlist"}).fillna(out["weekly_setup_bucket"])
    if "combined_bucket" in out.columns:
        out["overall_setup_label"] = out["combined_bucket"].map({"high_conviction_breakout":"Strong daily-weekly alignment","high_conviction_near_pivot":"Strong near-pivot alignment","building_setup":"Structure building","watchlist":"Watchlist"}).fillna(out["combined_bucket"])
    for col in ["daily_score","weekly_score","combined_score","final_daily_score","final_weekly_score","final_combined_score","rs_3m_pct","rs_6m_pct","industry_boost","combined_score_change","daily_score_change","weekly_score_change","rank_change","rs_rank","avg_combined_score"]:
        if col in out.columns: out[col] = pd.to_numeric(out[col], errors="coerce").round(1)
    if "avg_turnover_inr" in out.columns: out = out.drop(columns=["avg_turnover_inr"])
    return out
...
def filter_table(df: pd.DataFrame, search_text: str, industries: list[str], stages: list[str], min_score: float, score_col: str, label_col: str, labels: list[str]) -> pd.DataFrame:
    ...
    if search_text:
        ...
        out = out[mask]
    if industries and "Industry" in out.columns: out = out[out["Industry"].isin(industries)]
    if stages and "stage" in out.columns: out = out[out["stage"].isin(stages)]
    if labels and label_col in out.columns: out = out[out[label_col].isin(labels)]
    if score_col in out.columns: out = out[out[score_col].fillna(0) >= min_score]
...
def metric_row(combined: pd.DataFrame, daily: pd.DataFrame, weekly: pd.DataFrame, industry: pd.DataFrame, regime: pd.DataFrame, last_updated: str) -> None:
    ...
    c1.metric("Market regime", risk); c2.metric("Overall candidates", len(combined))
    ...
    c5.metric("Industries tracked", len(industry)); c6.metric("Last updated", last_updated)

def ranked_tab(title: str, df: pd.DataFrame, score_col: str, label_col: str) -> None:
    st.subheader(title)
    if df.empty: st.info("No scan outputs found yet."); return
...
def changes_tab(stock_changes: pd.DataFrame, top_movers: pd.DataFrame) -> None:
    ...
    if stock_changes.empty: st.info("No change data found yet."); return
    ...
    with c1:
        ...
        st.dataframe(stock_changes[stock_changes["new_daily_breakout"]][cols].sort_values("final_combined_score", ascending=False), use_container_width=True, hide_index=True, height=260)
    with c2:
        st.markdown("### New weekly breakout structures")
        cols = [c for c in ["ticker","Company Name","Industry","stage","final_combined_score","combined_score_change"] if c in stock_changes.columns]
        st.dataframe(stock_changes[stock_changes["new_weekly_breakout"]][cols].sort_values("final_combined_score", ascending=False), use_container_width=True, hide_index=True, height=260)
    with c3:
        ...
        st.dataframe(stock_changes[stock_changes["entered_stage_2"]][cols].sort_values("final_combined_score", ascending=False), use_container_width=True, hide_index=True, height=260)
...
def industry_rotation_tab(industry_changes: pd.DataFrame) -> None:
    ...
    if industry_changes.empty: st.info("No industry rotation data found yet."); return
    c1, c2 = st.columns(2)
    with c1:
        ...
        st.dataframe(industry_changes.sort_values(["rank_change","combined_score_change"], ascending=[False,False]).head(20)[cols], use_container_width=True, hide_index=True, height=360)
    with c2:
        st.markdown("### Falling industries")
        cols = [c for c in ["Industry","avg_combined_score","combined_score_change","rs_rank","rank_change","strong_combined","new_cluster"] if c in industry_changes.columns]
        st.dataframe(industry_changes.sort_values(["rank_change","combined_score_change"], ascending=[True,True]).head(20)[cols], use_container_width=True, hide_index=True, height=360)
...
def industry_tab(industry: pd.DataFrame) -> None:
    ...
    if industry.empty: st.info("No scan outputs found yet."); return
...
def stage_by_industry_tab(combined: pd.DataFrame) -> None:
    ...
    if combined.empty or "Industry" not in combined.columns or "stage" not in combined.columns: st.info("No stage data found yet."); return
...
outputs = {"daily": str(Path(outdir) / "vcp_daily_ranked.csv"), "weekly": str(Path(outdir) / "vcp_weekly_ranked.csv"), "combined": str(Path(outdir) / "vcp_combined_ranked.csv"), "industry": str(Path(outdir) / "industry_strength.csv"), "regime": str(Path(outdir) / "market_regime.csv"), "stock_changes": str(Path(outdir) / "stock_changes.csv"), "industry_changes": str(Path(outdir) / "industry_changes.csv"), "top_movers": str(Path(outdir) / "top_movers.csv"), "daily_charts_dir": str(Path(outdir) / "charts" / "daily"), "weekly_charts_dir": str(Path(outdir) / "charts" / "weekly")}
daily_df = add_safe_labels(safe_read(outputs["daily"])); weekly_df = add_safe_labels(safe_read(outputs["weekly"])); combined_df = add_safe_labels(safe_read(outputs["combined"])); industry_df = add_safe_labels(safe_read(outputs["industry"])); regime_df = safe_read(outputs["regime"]); stock_changes_df = add_safe_labels(safe_read(outputs["stock_changes"])); industry_changes_df = add_safe_labels(safe_read(outputs["industry_changes"])); top_movers_df = add_safe_labels(safe_read(outputs["top_movers"])); last_updated = get_last_updated(outputs["combined"])
...
tabs = st.tabs(["Dashboard","Overall","Daily","Weekly","What changed","Industry rotation","Industries","Stage by Industry","Stock detail","Help"])
with tabs[0]: dashboard_tab(combined_df, daily_df, weekly_df, industry_df, regime_df, last_updated)
with tabs[1]: ranked_tab("Overall candidates", combined_df, "final_combined_score", "overall_setup_label")
with tabs[2]: ranked_tab("Daily candidates", daily_df, "final_daily_score", "daily_setup_label")
with tabs[3]: ranked_tab("Weekly candidates", weekly_df, "final_weekly_score", "weekly_setup_label")
with tabs[4]: changes_tab(stock_changes_df, top_movers_df)
with tabs[5]: industry_rotation_tab(industry_changes_df)
with tabs[6]: industry_tab(industry_df)
with tabs[7]: stage_by_industry_tab(combined_df)
with tabs[8]: stock_detail_tab(combined_df, outputs["daily_charts_dir"], outputs["weekly_charts_dir"])
with tabs[9]: help_tab()
