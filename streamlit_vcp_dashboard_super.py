...
def _pdf_header(canvas, doc):
    ...
    canvas.drawString(doc.leftMargin, A4[1] - 28, "Watchlist Export")
    canvas.setFont("Helvetica", 8)
    canvas.setFillColor(colors.HexColor("#64748b"))
    canvas.drawRightString(A4[0] - doc.rightMargin, A4[1] - 28, f"Page {canvas.getPageNumber()}")
    canvas.restoreState()

def generate_watchlist_pdf_bytes(current_df: pd.DataFrame, selected_watchlist: str, daily_dir: str, weekly_dir: str) -> bytes:
    buffer = BytesIO()
    doc = SimpleDocTemplate(
        ...
        topMargin=36,
        bottomMargin=30,
        title=f"Watchlist - {selected_watchlist}",
        author="ChatGPT",
    )
    ...
    elements = []
    elements.append(Paragraph(f"Watchlist Export - {selected_watchlist}", title_style))
    elements.append(Paragraph("Generated from the current watchlist view, including summary, actions, ranks, and available chart images.", sub_style))

    if current_df is None or current_df.empty:
        elements.append(Paragraph("No watchlist rows available.", body_style))
        doc.build(elements, onFirstPage=_pdf_header, onLaterPages=_pdf_header)
        return buffer.getvalue()

    work = current_df.copy()
    total = len(work)
    buy_count = int(work["action"].astype(str).isin(["Buy", "Tactical Buy"]).sum()) if "action" in work.columns else 0
    short_count = int(work["action"].astype(str).isin(["Short", "Tactical Short"]).sum()) if "action" in work.columns else 0
    watch_count = int(work["action"].astype(str).str.contains("Watch", regex=False).sum()) if "action" in work.columns else 0

    stage_counts = work["stage"].value_counts() if "stage" in work.columns else pd.Series(dtype=int)
    summary_rows = [
        ["Portfolio", selected_watchlist],
        ["Stocks", str(total)],
        ["Buy / Tactical Buy", str(buy_count)],
        ["Short / Tactical Short", str(short_count)],
        ["Watch actions", str(watch_count)],
        ["Stage 1 / 2 / 3 / 4", f"{int(stage_counts.get('Stage 1', 0))} / {int(stage_counts.get('Stage 2', 0))} / {int(stage_counts.get('Stage 3', 0))} / {int(stage_counts.get('Stage 4', 0))}"],
    ]
    ...
    table_df = work.copy()
    table_df["Dataset Rank"] = pd.to_numeric(table_df.get("current_rank"), errors="coerce").fillna(-1).astype(int).astype(str).replace("-1", "-")
    preview_cols = [c for c in ["Company Name", "ticker", "action", "action_confidence", "Dataset Rank", "Industry", "stage"] if c in list(table_df.columns) + ["Dataset Rank"]]
    preview_data = [["Name", "Ticker", "Action", "Confidence", "Rank", "Industry", "Stage"]]
    for _, row in table_df.head(20).iterrows():
        preview_data.append([
            pdf_text(row.get("Company Name")),
            pdf_text(row.get("ticker")),
            pdf_text(row.get("action")),
            pdf_text(row.get("action_confidence")),
            pdf_text(row.get("Dataset Rank")),
            pdf_text(row.get("Industry")),
            pdf_text(row.get("stage")),
        ])
    elements.append(Paragraph("Portfolio action table", section_style))
    preview_table = Table(preview_data, colWidths=[110, 62, 82, 58, 38, 110, 55], repeatRows=1)
    ...
    chart_limit = min(50, len(work))
    for idx, (_, row) in enumerate(work.head(chart_limit).iterrows(), start=1):
        display = pdf_text(stock_display_label(row))
        rank_val = pd.to_numeric(row.get("current_rank"), errors="coerce")
        rank_txt = str(int(rank_val)) if pd.notna(rank_val) else pdf_text(get_stock_rank(row.get("ticker", "")))
        rank_change_val = pd.to_numeric(row.get("rank_change"), errors="coerce")
        rank_change_txt = "-"
        if pd.notna(rank_change_val) and rank_change_val != 0:
            rank_change_txt = f"{'up' if rank_change_val > 0 else 'down'} {abs(int(rank_change_val))}"

        elements.append(Paragraph(f"{idx}. {display}", section_style))
        meta_rows = [
            ["Ticker", pdf_text(row.get("ticker")), "Action", pdf_text(row.get("action", "No Trade"))],
            ["Dataset Rank", rank_txt, "Rank Change", rank_change_txt],
            ["Stage", pdf_text(row.get("stage")), "Label", pdf_text(row.get("label"))],
            ["Industry", pdf_text(row.get("Industry")), "Confidence", pdf_text(row.get("action_confidence"))],
            ["Rationale", pdf_text(row.get("rationale")), "Stop", pdf_text(row.get("stop_framework"))],
        ]
        meta_table = Table(meta_rows, colWidths=[72, 170, 72, 186])
        ...
        chart_cells = []
        for folder, suffix, label in [(daily_dir, "_daily.png", "Daily chart"), (weekly_dir, "_weekly.png", "Weekly chart")]:
            chart_path = resolve_chart_path(folder, row.get("ticker", ""), suffix)
            block = [Paragraph(label, small_style), Spacer(1, 3)]
            if chart_path and Path(chart_path).exists():
                block.append(RLImage(str(chart_path), width=250, height=155))
            else:
                block.append(Paragraph("Chart not available.", body_style))
            chart_cells.append(block)
        chart_table = Table([chart_cells], colWidths=[260, 260])
        ...
        if mini:
            ...
            elements.append(Paragraph(f"Signals: {mini}", body_style))
...
with tabs[4]:
    ...
    if not st.session_state["watchlist_names"]:
        ...
        if not current.empty:
            ...
            pdf_bytes = generate_watchlist_pdf_bytes(current.reset_index(drop=True), selected_watchlist, daily_dir, weekly_dir)
            st.download_button(
                "Export Watchlist PDF",
                data=pdf_bytes,
                file_name=f"watchlist_{str(selected_watchlist).lower().replace(' ', '_').replace('/', '_')}.pdf",
                mime="application/pdf",
                use_container_width=True,
                key=f"watchlist_pdf_{selected_watchlist}",
            )
