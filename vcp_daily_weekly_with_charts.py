...
DEFAULT_CONFIG = {
    ...
    "min_history": 300,
    "swing_order_daily": 8,
    "swing_order_weekly": 3,
    "max_contractions": 4,
    "pivot_lookback_daily": 30,
    "pivot_lookback_weekly": 10,
    "volume_short_window": 10,
    "volume_long_window": 50,
    "market_ma_fast": 50,
    "market_ma_slow": 200,
    "breakout_volume_ratio": 1.8,
    "near_pivot_min_pct": -5.0,
    "near_pivot_max_pct": 1.5,
    "recent_range_days": 10,
    "recent_range_max_pct": 8.0,
    "min_avg_turnover_inr": 5e7,
    "industry_boost_top": 80.0,
    "industry_boost_mid": 60.0,
    "industry_boost_low": 40.0,
    "industry_boost_top_points": 10.0,
    "industry_boost_mid_points": 5.0,
    "industry_boost_low_points": 2.0,
    "min_contraction_days_daily": 5,
    "min_contraction_days_weekly": 2,
    "min_contraction_depth_pct_daily": 4.0,
    "min_contraction_depth_pct_weekly": 5.0,
    "min_base_duration_days": 30,
    "min_base_duration_weeks": 8,
    "max_latest_contraction_pct": 10.0,
    "min_weekly_strength_score": 0.45,
}
...
def fetch_prices(tickers: List[str], period: str, interval: str = "1d", batch_size: int = 40) -> Dict[str, pd.DataFrame]:
    ...
    if retry_failed:
        print(f"Still failed after retry ({interval}): {len(retry_failed)}")
        print(retry_failed[:25])
    return out
...
def market_regime(index_df: pd.DataFrame, index_symbol: str, ma_fast: int, ma_slow: int) -> MarketRegime:
    ...
    if above_fast and above_slow and trend_up:
        label = "risk_on"
    elif above_slow:
        label = "mixed"
    else:
...
def analyze_symbol(ticker: str, df: pd.DataFrame, benchmark_df: pd.DataFrame, regime: MarketRegime, config: dict) -> Optional[VCPScoreCard]:
    ...
    return VCPScoreCard(
        ...
        ma200=round(ma200, 2),
        stage=stage,
        rs_3m_pct=round(float(rs_3m), 2) if pd.notna(rs_3m) else np.nan,
        rs_6m_pct=round(float(rs_6m), 2) if pd.notna(rs_6m) else np.nan,
        avg_turnover_inr=round(float(avg_turnover_inr), 2) if pd.notna(avg_turnover_inr) else np.nan,
        daily_setup_bucket=daily_bucket,
        daily_score=daily_score,
        daily_pivot=round(float(daily_pivot), 2) if pd.notna(daily_pivot) else np.nan,
        daily_breakout_distance_pct=round(float(daily_breakout_distance), 2) if pd.notna(daily_breakout_distance) else np.nan,
        daily_contraction_depths_pct=daily_depths,
        daily_contraction_durations=daily_durations,
        daily_contraction_score=round(float(daily_contraction_score_val), 2),
        daily_base_duration_days=round(float(daily_base_duration), 2),
        weekly_setup_bucket=weekly_bucket,
        weekly_score=weekly_score,
        weekly_pivot=round(float(weekly_pivot), 2) if pd.notna(weekly_pivot) else np.nan,
        weekly_breakout_distance_pct=round(float(weekly_breakout_distance), 2) if pd.notna(weekly_breakout_distance) else np.nan,
        weekly_contraction_depths_pct=weekly_depths,
        weekly_contraction_durations=weekly_durations,
        weekly_contraction_score=round(float(weekly_contraction_score_val), 2),
        weekly_base_duration_weeks=round(float(weekly_base_duration), 2),
        weekly_vcp_quality=weekly_quality,
        combined_bucket=combo_bucket,
        combined_score=combined_score,
        volume_dryup_ratio=round(float(volume_dryup_ratio), 2) if pd.notna(volume_dryup_ratio) else np.nan,
        breakout_volume_ratio=round(float(breakout_volume_ratio), 2) if pd.notna(breakout_volume_ratio) else np.nan,
        notes=", ".join(notes),
    )
...
def export_chart(df: pd.DataFrame, symbol: str, title: str, outfile: Path, pivot: Optional[float], setup_bucket: str, score: float, stage: str, is_weekly: bool = False) -> None:
    ...
    ma_fast = close.rolling(10 if is_weekly else 50).mean()
    ma_mid = close.rolling(30 if is_weekly else 150).mean()
    ma_slow = close.rolling(40 if is_weekly else 200).mean()
    ...
    ax1.plot(plot_df.index, ma_fast.values, label="MA Fast", linewidth=1.0)
    ax1.plot(plot_df.index, ma_mid.values, label="MA Mid", linewidth=1.0)
    ax1.plot(plot_df.index, ma_slow.values, label="MA Slow", linewidth=1.0)
